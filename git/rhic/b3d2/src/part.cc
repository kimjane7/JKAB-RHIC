...
void CPart::InitBalance(int IDset,double rxset,double ryset,double tauset,double etaset,double pxset,double pyset,double mset,double rapidityset,int weightset,bool realset,int balanceIDset){
	...
	if(reality && balanceID!=-1){
		printf("A. InitBalance, screwy, balanceID=%d\n",balanceID);
		exit(1);
	}
	if(!reality && balanceID==-1){
		printf("B. InitBalance, screwy fake particle, balanceID=%d\n",balanceID);
		exit(1);
	}
}
...
void CPart::Propagate(double tau){
	...
	if(currentmap==&(b3d->PartMap) && tau<b3d->TAUCOLLMAX && fabs(eta)>0.000001+b3d->ETAMAX && b3d->BJORKEN){
...
void CPart::FindActions(){
	...
	if(cell!=NULL){
		if(b3d->COLLISIONS && tau0<b3d->TAUCOLLMAX &&
		 ( (balanceID<0 && (nscatt<b3d->NSCATT_MAX || reality)) || (balanceID>=0 && nscatt<b3d->NSCATT_MAX))){
			FindCellExit();
			FindCollisions();
		}
		else{
